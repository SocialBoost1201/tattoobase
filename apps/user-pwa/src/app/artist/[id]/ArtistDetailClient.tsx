...
export default function ArtistDetailClient({ artist, works }: { artist: any; works: any[] }) {
  ...
  return (
    <div ref={containerRef} className="pb-32 md:pb-8">
      ...
      {/* カバー画像（PC: 全幅バナー） */}
      <div className="-mx-4 -mt-6 h-64 md:h-80 relative">
        <Image src={coverImage} alt="Cover" fill className="object-cover" priority sizes="100vw" />
        <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent" />
      </div>

      {/* PC: 2カラムレイアウト / モバイル: 1カラム */}
      <div className="md:grid md:grid-cols-[360px_1fr] md:gap-10 md:items-start">

        {/* ===== 左カラム: Sticky プロフィール + 予約 + 料金 ===== */}
        <div className="md:sticky md:top-32 md:space-y-6">
          ...
          <div className="hidden md:block px-2">
            <Link href={`/booking/start?artistId=${artist.id}`}
              className="w-full flex items-center justify-center gap-2 bg-white hover:bg-neutral-200 text-black font-extrabold text-sm py-4 rounded-full transition-all duration-300 font-heading tracking-wide">
        ...
        {/* ===== 右カラム: 作品グリッド ===== */}
        <div>
          <div className="h-px bg-neutral-900 my-6 stagger-fade md:hidden" />
                ...
                {works.map((w: any, index: number) => {
                  ...
                  return (
                    <button key={w.id} onClick={() => setSelectedWorkIndex(index)}
                      ...
                      <Image src={img} alt="work" fill sizes="(max-width:768px) 33vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-110" />
